...
namespace UnityEngine.ResourceManagement.ResourceProviders
{
    ...
    internal class AssetBundleResource : IAssetBundleResource
    {
        internal enum LoadType
        {
            ...
            Local,
            Web,
            ///// BEGIN PlayAssetDeliveryProxy
            AssetPack,
            ///// END PlayAssetDeliveryProxy
        }
        ...
        private bool WaitForCompletionHandler()
        {
            ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
            if (_playAssetPackRequest != null && !_playAssetPackRequest.IsDone)
            {
                // 不能同步加载，这里同步等待会死锁，因为 _playAssetPackRequest.IsDone 是在主线程设置，永远没机会修改
                throw new Exception($"WaitForCompletion() is not allowd for assetBundles in remote assetpacks {m_TransformedInternalId} {m_ProvideHandle.Location}");
            }
#endif
            ///// END PlayAssetDeliveryProxy
        ...
        internal static void GetLoadInfo(ProvideHandle handle, out LoadType loadType, out string path)
        {
            ...
            if (Application.platform == RuntimePlatform.Android && path.StartsWith("jar:"))
            {
                ...
                if (AssetBundleProvider.playAssetDeliveryProxy?.IsAssetInPack(bundleName) ?? false)
                {
                    loadType = LoadType.AssetPack;
                }
                else
        ...
        private void BeginOperation()
        {
            ...
            else
            {
                ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
                if (loadType == LoadType.AssetPack && AssetBundleProvider.playAssetDeliveryProxy != null)
                {
                    var bundleName = Path.GetFileNameWithoutExtension(m_TransformedInternalId);
                    var assetPackName = AssetBundleProvider.playAssetDeliveryProxy.GetPackByAssetName(bundleName);

                    // 这里不用处理同步加载的情况，在下一个循环里面 WaitForCompletionHandler() 会处理
                    _playAssetPackRequest = AssetBundleProvider.playAssetDeliveryProxy.RetrieveAssetPackAsync(assetPackName);
                    if (_playAssetPackRequest.IsDone)
                    {
                        OnPlayAssetPackRequestCompleted(bundleName, _playAssetPackRequest, assetPackName);
                        return;
                    }

                    // _playAssetPackRequest.Completed += request => OnPlayAssetPackRequestCompleted(bundleName, request);
                    MonoBehaviourCallbackHooks.Instance.StartCoroutine(_playAssetPackRequestCoroutine = CheckPlayAssetPackRequest(bundleName, _playAssetPackRequest, assetPackName));
                    return;
                }
#endif
                ///// END PlayAssetDeliveryProxy
        ...
        ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
        Google.Play.AssetDelivery.PlayAssetPackRequest _playAssetPackRequest;
        IEnumerator _playAssetPackRequestCoroutine;
        IEnumerator CheckPlayAssetPackRequest(string bundleName, Google.Play.AssetDelivery.PlayAssetPackRequest request, string assetPackName)
        {
            while (!request.IsDone)
            {
                // TODO 这里是否要弹提示可能要再考虑，玩家游玩过程中突然弹出下载提示有点突兀
                if (request.Status == Google.Play.AssetDelivery.AssetDeliveryStatus.WaitingForWifi)
                {
                    var userConfirmationOperation = AssetBundleProvider.playAssetDeliveryProxy.ShowCellularDataConfirmation();

                    // Wait for confirmation dialog action.
                    yield return userConfirmationOperation;

                    if ((userConfirmationOperation.Error != Google.Play.AssetDelivery.AssetDeliveryErrorCode.NoError) ||
                       (userConfirmationOperation.GetResult() != Google.Play.AssetDelivery.ConfirmationDialogResult.Accepted))
                    {
                    ...
                    yield return new WaitUntil(() => request.Status != Google.Play.AssetDelivery.AssetDeliveryStatus.WaitingForWifi);
                }
            ...
            _playAssetPackRequestCoroutine = null;
            OnPlayAssetPackRequestCompleted(bundleName, request, assetPackName);
        }

        private void OnPlayAssetPackRequestCompleted(string bundleName, Google.Play.AssetDelivery.PlayAssetPackRequest request, string assetPackName)
        {
            if (request.Error != Google.Play.AssetDelivery.AssetDeliveryErrorCode.NoError)
            {
                m_ProvideHandle.Complete<AssetBundleResource>(null, false, new RemoteProviderException(string.Format("Error downloading error pack: '{0}' '{1}'.", request.Error, m_TransformedInternalId), m_ProvideHandle.Location));
                return;
            }
            if (request.Status != Google.Play.AssetDelivery.AssetDeliveryStatus.Available)
            {
                m_ProvideHandle.Complete<AssetBundleResource>(null, false, new RemoteProviderException(string.Format("Error downloading status: '{0}' '{1}'.", request.Status, m_TransformedInternalId), m_ProvideHandle.Location));
                return;
            }

            var assetLocation = request.GetAssetLocation(bundleName);
            if (assetLocation == null)
            {
                ...
                if ("assetpack".Equals(bundleName.Split('/')[0]))
                {
            ...
            {
                m_RequestOperation = AssetBundle.LoadFromFileAsync(assetLocation.Path, m_Options == null ? 0 : m_Options.Crc, assetLocation.Offset);
            }

            // m_RequestOperation = request.LoadAssetBundleAsync(bundleName);
            if (m_RequestOperation == null)
            {
                m_ProvideHandle.Complete<AssetBundleResource>(null, false, new RemoteProviderException(string.Format("AssetBundle not found in assetpack : '{0}' '{1}' '{2}'.", bundleName, assetPackName, m_TransformedInternalId), m_ProvideHandle.Location));
                return;
            }
            else
                AddCallbackInvokeIfDone(m_RequestOperation, LocalRequestOperationCompleted);
        ...
        public void Unload()
        {
            ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
            if (_playAssetPackRequest != null)
            {
                _playAssetPackRequest.AttemptCancel();
                _playAssetPackRequest = null;
            }
            if (_playAssetPackRequestCoroutine != null)
            {
                MonoBehaviourCallbackHooks.Instance.StopCoroutine(_playAssetPackRequestCoroutine);
                _playAssetPackRequestCoroutine = null;
            }
#endif
            ///// END PlayAssetDeliveryProxy
    ...
    public class AssetBundleProvider : ResourceProviderBase
    {
        ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
        public static IPlayAssetDeliveryProxy playAssetDeliveryProxy;
#endif
        ///// END PlayAssetDeliveryProxy
    ...
    ///// BEGIN PlayAssetDeliveryProxy
#if UNITY_ANDROID
    public interface IPlayAssetDeliveryProxy
    {
        bool IsPack(string name);
        bool IsAssetInPack(string assetName);
        string GetPackByAssetName(string assetName);
        List<string> GetAllPacks();
        Google.Play.AssetDelivery.PlayAssetBundleRequest RetrieveAssetBundleAsync(string assetBundleName);
        Google.Play.AssetDelivery.PlayAssetPackRequest RetrieveAssetPackAsync(string assetPackName);
        Google.Play.Common.PlayAsyncOperation<Google.Play.AssetDelivery.ConfirmationDialogResult, Google.Play.AssetDelivery.AssetDeliveryErrorCode> ShowCellularDataConfirmation();
    }
#endif
    ///// END PlayAssetDeliveryProxy
}
