...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            // modified by ZYL
            m_RainDrop            = stack.GetComponent<RainDrop>();
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            // Modified by ZYL
            if (m_RainDrop.IsActive() && !cameraData.isSceneViewCamera)
            {
                using (new ProfilingScope(cmd, ProfilingSampler.Get(CustomProfileId.RainDrop)))
                {
                    DoRainDrop(cameraData.camera, cmd, GetSource(), GetDestination());
                    Swap();
                }
            }
        ...
        #region RainDrop
        // Modified by ZYL
        void DoRainDrop(Camera camera, CommandBuffer cmd, int source, int destination)
        {
            var material = m_RainDrop.material.value;

            cmd.SetGlobalTexture("_BlitTex", source);
            material.SetTexture("_BumpMap", m_RainDrop.GenerateTexture());
